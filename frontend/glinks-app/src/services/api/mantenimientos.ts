...
export const mantenimientosApi = {
  ...
  async createPhysical(data: Omit<CreateMaintenanceInput, "legalClientId">) {
    ...
    if (!responsibleId) {
      throw new Error("No se pudo identificar el usuario responsable");
    }
...
// Función para verificar si un producto está en algún mantenimiento
export async function checkProductInMaintenances(productId: string): Promise<boolean> {
  ...
  const allMaintenances = [...(physical?.data ?? []), ...(legal?.data ?? [])];
  
  return allMaintenances.some(m => 
    m.maintenanceProducts?.some(mp => mp.product_id === productId)
  );
}

// Función para obtener los mantenimientos de un producto
